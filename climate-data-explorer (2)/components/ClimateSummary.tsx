...
import type { WeatherDataPoint, Thresholds, ClimateStats } from '../types';
import { Card } from './Card';
import { RainfallIcon, ThermometerIcon, CalendarDaysIcon } from './icons/WeatherIcons';
import { getClimateSummary } from '../services/geminiService';
...
export const ClimateSummary: React.FC<ClimateSummaryProps> = ({ weatherData, thresholds, loading }) => {
  ...
  useEffect(() => {
    if (stats) {
      ...
      // This will wait for 1 second of inactivity before fetching the summary.
      const handler = setTimeout(() => {
        // --- AI Summary Temporarily Disabled ---
        // The following call to the Gemini API has been commented out to prevent
        // rate-limiting errors (HTTP 429).
        setGeminiSummary("AI summary is temporarily unavailable due to high request volume. Please try again later.");
        setSummaryLoading(false);
        /*
        getClimateSummary(stats)
          .then(summary => {
            setGeminiSummary(summary);
          })
          .catch(err => {
            console.error(err);
             // Provide a specific, user-friendly message for rate limit errors.
            if (err instanceof Error && (err.message.includes('429') || err.message.includes('RESOURCE_EXHAUSTED'))) {
              setGeminiSummary("AI summary is temporarily unavailable due to high request volume. Please try again in a minute.");
            } else {
              setGeminiSummary("Could not load AI summary.");
            }
          })
          .finally(() => {
            setSummaryLoading(false);
    ...
    <Card title="Climate Summary">
        <div className="space-y-4">
            <div className="mb-4 p-3 bg-slate-900/50 rounded-lg">
              <h3 className="font-semibold text-sm text-blue-300 mb-1">AI-Powered Insight</h3>
              {summaryLoading ? (
                <div className="animate-pulse h-4 bg-slate-700 rounded w-3/4"></div>
              ) : (
                geminiSummary && <p className="text-slate-300 italic">"{geminiSummary}"</p>
              )}
            </div>
